...
int main()
{
    ...
    //设置定时函数，第一个值为毫秒数
    std::shared_ptr<std::function<int(const UINT64&& num,const std::string&& name,  std::function<void()>&)>> 
        setTimerFuncPtr=
        std::make_shared<std::function<int(const UINT64&& num,const std::string && name,  std::function<void()>&)>>
    ...
    //添加rpc函数
    std::shared_ptr<std::function<int(const std::string&& funcName, std::function<int(const std::string&, std::string&)>&)>>
        setRPCFuncPtr=
        std::make_shared<std::function<int(const std::string && funcName, std::function<int(const std::string&, std::string&)>&)>>
    ...
    std::shared_ptr<std::function<int(const std::string&& funcName, std::any&)>>  
        setDllFuncPtr=
        std::make_shared<std::function<int(const std::string && funcName, std::any&)>>
    ...
    //添加消息
    std::shared_ptr<std::function<int(const std::string&& queueName, std::shared_ptr<MessageBase>&)>>
        pushMessagePtr =
        std::make_shared<std::function<int(const std::string && queueName, std::shared_ptr<MessageBase>&)>>
        (std::bind(&TaskScheduler::AddMessage, taskScheduler, std::placeholders::_1, std::placeholders::_2));
    //添加消息处理函数
    std::shared_ptr<std::function<int(const std::string&&, const int&&, const std::string&&, std::function<void(std::shared_ptr<MessageBase>)>&)>>  
        pushCallbackFuncPtr=
        std::make_shared<std::function<int(const std::string&&, const int&&, const std::string&&, std::function<void(std::shared_ptr<MessageBase>)>&)>>
